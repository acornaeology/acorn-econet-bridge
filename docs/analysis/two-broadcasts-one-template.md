...
All of this is called through [`transmit_frame_a`](address:E517@variant_1?hex) or its byte-for-byte mirror [`transmit_frame_b`](address:E4C0@variant_1?hex), which read from the same buffer via the zero-page pointer `mem_ptr_lo`/`mem_ptr_hi`. The transmit routines don't care which buffer they're reading from or what it contains; they just push bytes into their ADLC's TX FIFO until the end-pointer is reached.
...
- [`build_announce_b`](address:E458@variant_1?hex) — the template builder.
- [`transmit_frame_a`](address:E517@variant_1?hex) and [`transmit_frame_b`](address:E4C0@variant_1?hex) — the mirrored transmit routines.
- The outbound frame control block at `tx_dst_stn..tx_data0` (`&045A-&0460`).
- The reset sequence calling the pair at [`&E038`](address:E038@variant_1) onward.
