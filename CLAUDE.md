...
- **Independent cross-reference.** J.G. Harston's [BRIDGE.SRC](https://mdfs.net/System/ROMs/Econet/Bridge/BRIDGE.SRC) is a BBC-BASIC-embedded disassembly of the same variant, independently reached. His code assembles to the same addresses as ours, so his label names and comments provide a second opinion on the semantics. Refinements informed by his source are paraphrased rather than copied verbatim; his protocol names (BridgeReset / BridgeReply / WhatNet / IsNet / BridgeQuery port) are adopted as canonical for cross-referenceability.
