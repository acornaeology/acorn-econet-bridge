"""CLI entry point for acorn-econet-bridge-disasm-tool."""
...
def cmd_disassemble(args):
    ...
    env = os.environ.copy()
    env["ACORN_ECONET_BRIDGE_ROM"] = str(version_dirpath / "rom" / f"{prefix}-{args.version}.rom")
    env["ACORN_ECONET_BRIDGE_OUTPUT"] = str(version_dirpath / "output")
...
def cmd_find_shared(args):
    """Find shared 6502 code fragments between the project ROM and references."""
    from disasm_tools.find_shared import find_shared

    version_dirpath = get_version_dirpath(args.version)
    prefix = rom_prefix(version_dirpath)
    rom_filepath = version_dirpath / "rom" / f"{prefix}-{args.version}.rom"

    # Read load address from py8dis JSON output (falls back to mos6502 constant)
    try:
        import json
        from disasm_tools.mos6502 import ROM_BASE
        json_filepath = version_dirpath / "output" / f"{prefix}-{args.version}.json"
        if json_filepath.exists():
            meta = json.loads(json_filepath.read_text()).get("meta", {})
            load_addr = meta.get("load_addr", ROM_BASE)
        else:
            load_addr = ROM_BASE
    except Exception:
        from disasm_tools.mos6502 import ROM_BASE
    ...
    primary_spec = f"{prefix}-{args.version}={rom_filepath}@0x{load_addr:04X}"
    sys.exit(find_shared(primary_spec, args.references, args.min_len, args.limit))


def main():
    parser = argparse.ArgumentParser(
        prog="acorn-econet-bridge-disasm-tool",
        description="Acorn Econet Bridge ROM disassembly tools",
    )
    ...
    dis_parser.add_argument("version", help="Version (e.g. 1)")
    dis_parser.set_defaults(func=cmd_disassemble)

    verify_parser = subparsers.add_parser(
        "verify", help="Verify disassembly round-trips to original ROM"
    )
    verify_parser.add_argument("version", help="Version (e.g. 1)")
    verify_parser.set_defaults(func=cmd_verify)

    lint_parser = subparsers.add_parser(
        "lint", help="Validate annotation addresses in driver script"
    )
    lint_parser.add_argument("version", help="Version (e.g. 1)")
    lint_parser.set_defaults(func=cmd_lint)

    compare_parser = subparsers.add_parser(
        "compare", help="Compare two ROM versions"
    )
    compare_parser.add_argument("version_a", help="First version (e.g. 1)")
    compare_parser.add_argument("version_b", help="Second version (e.g. 2)")
    compare_parser.set_defaults(func=cmd_compare)

    extract_parser = subparsers.add_parser(
        "extract", help="Extract assembly section by address or label"
    )
    extract_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    audit_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    insert_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    cfg_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    context_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    labels_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    check_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    rename_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    backfill_parser.add_argument("source", help="Source version (e.g. 1)")
    backfill_parser.add_argument("target", help="Target version (e.g. 2)")
    ...
    promote_parser.add_argument("version", help="Version (e.g. 1)")
    ...
    shared_parser = subparsers.add_parser(
        "find-shared",
        help="Find shared 6502 code fragments vs. reference ROMs",
    )
    shared_parser.add_argument("version", help="Project ROM version (e.g. 1)")
    shared_parser.add_argument(
        "references", nargs="+",
        help=(
            "Reference ROM specs ([label=]path@load-addr). "
            "Example: mos120=/Users/rjs/Code/os120/os120.bin@&C000"
        ),
    )
    shared_parser.add_argument("--min-len", type=int, default=8,
                               help="Minimum matching instruction span (default: 8)")
    shared_parser.add_argument("--limit", type=int, default=None,
                               help="Show at most N longest matches per reference")
    shared_parser.set_defaults(func=cmd_find_shared)
